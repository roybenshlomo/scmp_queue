...
void lockless_stack<T>::push(const T & data)
{
    ...
    do {
        ...
        &m_head, &new_node->m_next, new_node, std::memory_order_release, std::memory_order_relaxed));
}
...
T lockless_stack<T>::pop()
{
    ...
    do {
        ...
        &m_head, &current_head, current_head->m_next, std::memory_order_release, std::memory_order_relaxed));
